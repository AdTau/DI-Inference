...
function  DI=compute_DI_M(X,Y,Nx,D,Dxy,alg,start_ratio,prob,flag)%[MI, DI, rev_DI]=compute_DI_MI(X,Y,Nx,D,alg,start_ratio,prob,flag)
...
pxy=pxy(:,length(pxy)-length(px)+1:end);%---> APAÑO!!!!
for i_x=1:Nx
    px_xy(i_x,:)=pxy(i_x,:);
    for j=2:Nx
...
%DI= cumsum(temp_DI((floor(n_data*start_ratio)+1):end))/(n_data-floor(n_data*start_ratio));
DI =temp_DI;%cumsum(temp_DI)/n_data;
%DI=DI(end);
%rev_DI=cumsum(temp_rev_DI((floor(n_data*start_ratio)+1):end))/(n_data-floor(n_data*start_ratio));
%MI = DI+rev_DI;
